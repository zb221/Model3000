...
typedef struct
{
	union
	{
		struct
		{
			unsigned short recept_ok				 :1;				//1  “ª÷° ˝æ›Ω” ‹≥…π¶
			unsigned short recept_data_error :1;				//1  “ª÷° ˝æ›”– ˝æ›“Ï≥£
			unsigned short recept_crc_error  :1;				//1  “ª÷° ˝æ›crcºÏ—È“Ï≥
			unsigned short recept_write			 :1;				//1  “ª÷° ˝æ›Ω” ‹≥…π only	 write			
			}ubit;
		unsigned short all;	
	...
	unsigned char recept_point;//Õ®—∂”√Ω” ‹ ˝æ›”Œ±Í
	unsigned char recept_data_buffer[8];//Õ®—∂”√Ω” ‹ ˝æ›
	unsigned char recept_function_code;//03 ∂¡÷∏∂®ºƒ¥Ê∆˜£ª06–¥ºƒ¥Ê∆˜
	unsigned short calculate_crc;//Ω” ‹°¢∑¢ÀÕ ˝æ›º∆À„CEC÷µ
	unsigned char send_buffer[260];
	unsigned char spi_flash_buffer[256];//flash  ˝æ›ª∫¥
	unsigned short function_point;			    //1  “ª÷° ˝æ›Ω” ‹≥…π only	 write
...
typedef struct
{
	...
	union
	{
		struct
		{
			unsigned char lo;//2017.06.16∏ﬂ8µÕ8µ˜ªª
			unsigned char hi;		
			}ubit;	
		unsigned short hilo;
...
{		
	...
	union
	{
		struct
		{
			...
			unsigned short relay1				  		:1;		//1 ¥•∑¢
			unsigned short relay2				  		:1;		//1 ¥•∑¢	
			unsigned short relay3				  		:1;		//1 ¥•∑¢	
...
////		}RTC;		
	...
	unsigned short operating_mode;//151	 ≤Œ’’–≠“È
...
VARIABLE_EXT  RUN_PARAMETER  run_parameter;//moudlebus Ω”ø⁄”√ ˝æ›¡ø		
